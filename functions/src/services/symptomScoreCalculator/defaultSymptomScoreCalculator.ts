...
interface SymptomQuestionnaireLinkIds {
  question1a: string
  question1b: string
  question1c: string
  question2: string
...
export class DefaultSymptomScoreCalculator implements SymptomScoreCalculator {
  ...
  calculate(response: FHIRQuestionnaireResponse): SymptomScore {
    ...
    const result: SymptomScore = {
      ...
      dizzinessScore: this.value(response, linkIds.question9),
    }

    const q1NonMissing = [
      this.value(response, linkIds.question1a),
      this.value(response, linkIds.question1b),
      this.value(response, linkIds.question1c),
    ].filter((x) => x !== 0)
    ...
    const response2 = this.value(response, linkIds.question2)
    const response3 = this.value(response, linkIds.question3)
    const response4 = this.value(response, linkIds.question4)
    const response5 = this.value(response, linkIds.question5)
    ...
    const response6 = this.value(response, linkIds.question6)
    const response7 = this.value(response, linkIds.question7)
    ...
    const q8NonMissing = [
      this.value(response, linkIds.question8a),
      this.value(response, linkIds.question8b),
      this.value(response, linkIds.question8c),
    ].filter((x) => x !== 0)
  ...
  private value(response: FHIRQuestionnaireResponse, linkId: string): number {
