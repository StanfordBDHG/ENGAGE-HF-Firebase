...
export class FirebaseService {
    ...
    async getBloodPressureObservations(userId: string) {
        return this.getCollection<FHIRObservation>(`users/${userId}/bloodPressureObservations`)
    }

    async getBodyWeightObservations(userId: string) {
        return this.getCollection<FHIRObservation>(`users/${userId}/bodyWeightObservations`)
    }

    async getHeartRateObservations(userId: string) {
        return this.getCollection<FHIRObservation>(`users/${userId}/heartRateObservations`)
    }
    ...
    private async getCollection<T>(path: string): Promise<FirebaseDocument<T>[]> {
        const collection = await this.firestore.collection(path).get()
        return collection.docs.map(doc => ({ id: doc.id, content: doc.data() as T }))
    }
